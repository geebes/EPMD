...
lgnd = {'Neutral, surface transport',...
        ...
        'Selective, depth-integrated transport',...
        'Adaptive, depth-integrated transport, 1% trait-diffusion',...
        'Adaptive, depth-integrated transport, 10% trait-diffusion'};
...
%     t.Position(1)=7/365;
    ...
    if ~contains(input_filename{i},'_seasonal')
...
switch xscale
    case 'log'
        ...
        sname=['~/GitHub/EPMD_description/EPMD_Figures/Figure_1.png'];
    case 'lin'
        legend(lgnd,'Location','East')
        sname=['~/GitHub/EPMD_description/EPMD_Figures/cumulative_connections_linear.png'];
end
% subplot(212);
xlabel('Time')
...
%             'YTickLabel',{'10^{0}','10^{5}','10^{10}','10^{15}','10^{20}','10^{25}'})
    ...
    pp=pp+1;
    sh3=subplot(4,3,pp);
    [ab,abind]=sort(x(ocean.sample_points,1:94),2,'descend');
    
%     s = exp(-((ocean.ann_theta(ocean.sample_points)-ocean.ann_theta(ocean.sample_points)')./run_options{i}.w).^2);
%     for j=1:94
%         scatter(1:94,ab(j,:),15,s(j,abind(j,:)),'filled');
%         hold on
%     end
%     colorbar
...
sname=['~/GitHub/EPMD_description/EPMD_Figures/RankAbundance.png'];
set(gcf,'Color','w')
export_fig(sname,'-r300')
...
prc=95;
t_immigration = prctile(t_occ{3},prc,2)./prctile(t_occ{1},prc,2);
t_emmigration = prctile(t_occ{3},prc,1)./prctile(t_occ{1},prc,1);
...
sname=['~/GitHub/EPMD_description/EPMD_Figures/surface_vs_depth_map.png'];
set(gcf,'Color','w')
export_fig(sname,'-r300')
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
f3=figure(3);
...
sname=['~/GitHub/EPMD_description/EPMD_Figures/connection_matrix.png'];
set(gcf,'Color','w')
export_fig(sname,'-r300')
...
prc=95;
for i=[1 3]
    t_occ{i}(t_occ{i}==min(t_occ{i}(:)))=NaN;
...
sname=['~/GitHub/EPMD_description/EPMD_Figures/imm_vs_em.png'];
set(gcf,'Color','w')
export_fig(sname,'-r300')
