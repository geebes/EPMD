...
x3=reshape(full(x),nxr,nphen,nxc./nphen);
% sum across all phenotypes in each seed population
x2=squeeze(sum(x3,2));
% remove resident
x2=x2(:,2:end);
...
if true
    ...
    ii=ii(round(linspace(1,nxc/nphen-1,64)))';
    c=0;
    for N=ii
        c=c+1;
        subplot(8,8,c)
...
f103=figure(103);
clf
f103.Position=[52 1 2509 1344];

for i=1:size(x3,2)
    
    subplot(9,9,i)
    ax1=axesm ('mollweid','frame','on','FlineWidth',0.5,'MapLonLimit',[-245 115],'MapLatLimit',[-90 90]);
    ax1.Position=ax1.Position+[-0.1 -0.2 2 4].*0.00625;
    ...
    scatterm(ylat(iK(ispc)),xlon(iK(ispc)),10,'r','filled')
    
    th=title(['T_{opt} = ' num2str(T_opt(i)) '^\circC']);
    th.Position(1:2)=th.Position(1:2)-[2 0.65];
    axis off;
    drawnow
end
...
cmap=repmat(hot(nphen),nxc/nphen-1,1);
set(gcf,'DefaultAxesColorOrder',lhsdesign(341,3))

for i=1:9
    xsite1=squeeze(xyears(i,nphen+1:end,:));
    xsite=reshape(xsite1,nphen,nxc./nphen-1,[]);
    ...
    % plot phenotypes
    hold on
    h=plot(cumsum(xsite1)','LineWidth',0.25);
    set(h, {'color'},num2cell(cmap,2));
    % 
    plot(cumsum(xsum)','k','LineWidth',1)
